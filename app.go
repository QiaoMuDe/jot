...
// getWindowsOptions 读取用户保存的主题设置，返回 Windows 窗口标题栏和边框的配色方案
// 使窗口装饰（标题栏/边框）与主题风格保持一致
func (a *App) getWindowsOptions() *windows.Options {
	theme := a.settingService.Get("theme")
	switch theme {
	case "monokai-pro":
		return &windows.Options{
			...
			CustomTheme: &windows.ThemeSettings{
				DarkModeTitleBar:  windows.RGB(34, 31, 34),    // --topbar-bg: #221F22
				DarkModeTitleText: windows.RGB(252, 252, 250), // --text-primary: #FCFCFA
				DarkModeBorder:    windows.RGB(60, 58, 61),    // --border: #3C3A3D
			},
		}
	case "tokyo-night":
		return &windows.Options{
			Theme: windows.Dark,
			CustomTheme: &windows.ThemeSettings{
				DarkModeTitleBar:  windows.RGB(31, 35, 53),    // --topbar-bg: #1F2335
				DarkModeTitleText: windows.RGB(192, 202, 245), // --text-primary: #C0CAF5
				DarkModeBorder:    windows.RGB(47, 53, 74),    // --border: #2F354A
			},
		}
	case "dark":
		return &windows.Options{
			Theme: windows.Dark,
			CustomTheme: &windows.ThemeSettings{
				DarkModeTitleBar:  windows.RGB(20, 20, 20),    // --topbar-bg: #141414
				DarkModeTitleText: windows.RGB(232, 232, 232), // --text-primary: #E8E8E8
				DarkModeBorder:    windows.RGB(42, 42, 42),    // --border: #2A2A2A
			},
		}
	case "nord":
		return &windows.Options{
			Theme: windows.Light,
			CustomTheme: &windows.ThemeSettings{
				LightModeTitleBar:  windows.RGB(255, 255, 255), // --topbar-bg: #FFFFFF
				LightModeTitleText: windows.RGB(46, 52, 64),    // --text-primary: #2E3440
				LightModeBorder:    windows.RGB(216, 222, 233), // --border: #D8DEE9
			},
		}
	case "light":
		return &windows.Options{
			...
			CustomTheme: &windows.ThemeSettings{
				LightModeTitleBar:  windows.RGB(255, 255, 255), // --topbar-bg: #FFFFFF
				LightModeTitleText: windows.RGB(26, 26, 26),    // --text-primary: #1A1A1A
				LightModeBorder:    windows.RGB(238, 238, 238), // --border: #EEEEEE
			},
		}
	default: // "default" 及未知主题
		return &windows.Options{
			Theme: windows.Light,
			CustomTheme: &windows.ThemeSettings{
				LightModeTitleBar:  windows.RGB(255, 255, 255), // --topbar-bg: #FFFFFF
				LightModeTitleText: windows.RGB(45, 42, 36),    // --text-primary: #2D2A24
				LightModeBorder:    windows.RGB(229, 224, 216), // --border: #E5E0D8
			},
		}
	}
}
...
const (
	...
	dwmwaBorderColor          = 34     // DWMWA_BORDER_COLOR
	dwmwaCaptionColor         = 35     // DWMWA_CAPTION_COLOR
	dwmwaTextColor            = 36     // DWMWA_TEXT_COLOR
...
// findMainWindow 通过 Wails 窗口类名查找主窗口 HWND
...
// setWindowAttribute 调用 DwmSetWindowAttribute 设置 DWM 窗口属性
func setWindowAttribute(hwnd uintptr, attr int32, value unsafe.Pointer) {
	_, _, _ = procDwmSetWindowAttr.Call(hwnd, uintptr(attr), uintptr(value), unsafe.Sizeof(int32(0)))
}

// ApplyWindowTheme 根据主题名称动态切换 Windows 窗口的标题栏深色/浅色模式及自定义颜色
// 前端切换主题时调用此方法，使窗口装饰（标题栏/边框）与主题实时同步
func (a *App) ApplyWindowTheme(themeName string) {
	// 获取主题对应的配色
	isDark := false
	var titleBarColour, titleTextColour, borderColour int32
	switch themeName {
	case "monokai-pro":
		isDark = true
		titleBarColour = windows.RGB(34, 31, 34)     // --topbar-bg: #221F22
		titleTextColour = windows.RGB(252, 252, 250) // --text-primary: #FCFCFA
		borderColour = windows.RGB(60, 58, 61)       // --border: #3C3A3D
	case "tokyo-night":
		isDark = true
		titleBarColour = windows.RGB(31, 35, 53)     // --topbar-bg: #1F2335
		titleTextColour = windows.RGB(192, 202, 245) // --text-primary: #C0CAF5
		borderColour = windows.RGB(47, 53, 74)       // --border: #2F354A
	case "dark":
		isDark = true
		titleBarColour = windows.RGB(20, 20, 20)     // --topbar-bg: #141414
		titleTextColour = windows.RGB(232, 232, 232) // --text-primary: #E8E8E8
		borderColour = windows.RGB(42, 42, 42)       // --border: #2A2A2A
	case "nord":
		titleBarColour = windows.RGB(255, 255, 255) // --topbar-bg: #FFFFFF
		titleTextColour = windows.RGB(46, 52, 64)   // --text-primary: #2E3440
		borderColour = windows.RGB(216, 222, 233)   // --border: #D8DEE9
	case "light":
		titleBarColour = windows.RGB(255, 255, 255) // --topbar-bg: #FFFFFF
		titleTextColour = windows.RGB(26, 26, 26)   // --text-primary: #1A1A1A
		borderColour = windows.RGB(238, 238, 238)   // --border: #EEEEEE
	default: // "default" 及未知主题
		titleBarColour = windows.RGB(255, 255, 255) // --topbar-bg: #FFFFFF
		titleTextColour = windows.RGB(45, 42, 36)   // --text-primary: #2D2A24
		borderColour = windows.RGB(229, 224, 216)   // --border: #E5E0D8
	}
	...
	// 1. 设置深色/浅色模式 (DwmSetWindowAttribute, Win10 18985+)
	darkModeVal := int32(0)
	if isDark {
		darkModeVal = 1
	}
	setWindowAttribute(hwnd, dwmwaUseImmersiveDarkMode, unsafe.Pointer(&darkModeVal))

	// 2. 设置自定义标题栏颜色 (Win11 22000+)
	setWindowAttribute(hwnd, dwmwaCaptionColor, unsafe.Pointer(&titleBarColour))
	setWindowAttribute(hwnd, dwmwaTextColor, unsafe.Pointer(&titleTextColour))
	setWindowAttribute(hwnd, dwmwaBorderColor, unsafe.Pointer(&borderColour))

	// 3. 强制标题栏重绘 (修复 Win10 上设置后不立即生效的问题)
	// WM_NCACTIVATE 会触发 DWM 重新绘制非客户区（标题栏+边框）
	_, _, _ = procSendMessageW.Call(hwnd, wmNCACTIVATE, 0, 0)
	_, _, _ = procSendMessageW.Call(hwnd, wmNCACTIVATE, 1, 0)
}
