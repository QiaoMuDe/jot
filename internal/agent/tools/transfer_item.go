...
// 本文件实现 transfer_item 工具：在 AI 助手工作目录（~/.jot/workspace）与用户
...
// 源基名（fsFinalTarget，对齐 copy_item 智能路径语义）。
...
//   - upload（写工作区）对齐 copy_item：纯新增免审批；覆盖已存在目标时审批
...
// transfer_item 两端沙箱边界标识（报错文案用）
const (
	transferLabelWorkspace = "~/.jot/workspace"
	transferLabelDesktop   = "~/Desktop"
)

// transferItemTool 工作目录与用户桌面之间传输文件/目录的工具。
type transferItemTool struct {
	fsToolBase
}

var _ tool.InvokableTool = (*transferItemTool)(nil)
var _ ActionTextProvider = (*transferItemTool)(nil)

// ActionText 提供 tool_start 动作文案（实现 ActionTextProvider）。
func (t *transferItemTool) ActionText(argumentsInJSON string) string {
	var args struct {
	...
	case "download":
		if p != "" {
			return "下载项：" + TruncateRunes(p, 30)
		}
		return "下载项"
	case "upload":
		if p != "" {
			return "上传项：" + TruncateRunes(p, 30)
		}
		return "上传项"
	}
	return "传输项"
...
func (t *transferItemTool) Info(_ context.Context) (*schema.ToolInfo, error) {
...
func (t *transferItemTool) InvokableRun(ctx context.Context, argumentsInJSON string, _ ...tool.Option) (string, error) {
	// 用户取消检查
	if ctx.Err() != nil {
		return "", ctx.Err()
	}
	...
	if err := json.Unmarshal([]byte(argumentsInJSON), &args); err != nil {
		return "", fmt.Errorf("解析 transfer_item 参数失败: %w", err)
	}
	direction := strings.TrimSpace(args.Direction)
	if direction != "download" && direction != "upload" {
		return "", errors.New("transfer_item 参数 direction 非法，仅支持 download（工作目录→桌面）或 upload（桌面→工作目录）")
	}
	p := strings.TrimSpace(args.Path)
	if p == "" {
		return "", errors.New("transfer_item 参数缺少 path")
	}
	...
	if _, err := os.Lstat(srcFull); err != nil {
		if os.IsNotExist(err) {
			return "", errors.New("transfer_item 源文件/目录不存在：" + p)
		}
	...
	if destExists && !args.Overwrite {
		return "", errors.New("transfer_item 目标已存在，未设置 overwrite=true 时拒绝覆盖（覆盖请显式传 overwrite=true）")
	}

	// 审批检查点：download 写桌面（工作目录之外的外部副作用）一律审批 critical=true；
	// upload 写工作区对齐 copy_item（覆盖审批 critical=false，纯新增免审批）。
	...
	case direction == "download":
		...
		if err := t.requestApproval(ctx, "transfer_item", summary, true); err != nil {
			return "", err
		}
	case destExists:
		if err := t.requestApproval(ctx, "transfer_item", "上传并覆盖："+p+" → "+relTo(dstRoot, finalDst), false); err != nil {
			return "", err
		}
	}
...
// NewTransferItem 创建 transfer_item 工具。
func NewTransferItem(ctx *Context) tool.InvokableTool {
	return &transferItemTool{fsToolBase: fsToolBase{ctx: ctx}}
}
