...
- **AI 助手** — eino 驱动，OpenAI 兼容端点，纯 Agent 模式自动调度工具，流式输出、深度思考、14 个内置工具（管理笔记/待办/标签/笔记本/JSON 处理/网页读取/长文本摘要等），MCP 服务器扩展（设置页完整管理 + 连接池预热 + 断线重连），每个工具可单独启用/禁用，向量+关键词混合召回
- **数据管理** — 数据统计、向量索引、批量管理、待办清单、备份/还原、数据库瘦身
- **设计系统** — 14 套主题、纯手写 CSS、统一滚动条与弹性动效
