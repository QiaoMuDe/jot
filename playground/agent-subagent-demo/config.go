...
type Config struct {
	...
	Model   string // 模型名称
	Emit    bool   // 是否开启 EmitInternalEvents（子 Agent 内部事件转发）
}
...
func parseConfig() *Config {
	...
	flag.BoolVar(&cfg.Emit, "emit", envBool("AGENT_DEMO_EMIT_INTERNAL", false),
		"开启 EmitInternalEvents：子 Agent 内部事件实时转发给顶层（环境变量 AGENT_DEMO_EMIT_INTERNAL）")
	flag.Parse()
...
// envBool 读取布尔环境变量（1/true/yes 视为 true），未设置时返回默认值。
...
func printConfigSummary(cfg *Config) {
	...
	fmt.Printf("EmitInternalEvents : %v\n", cfg.Emit)
	fmt.Println("====================================")
}
