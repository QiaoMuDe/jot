...
- **AI 助手** — eino 驱动，OpenAI 兼容端点，Chat / Agent / Plan 三态模式（Chat 纯对话、Agent 自动调度工具、Plan 先规划后执行），流式输出、深度思考、13 项 AI 技能（翻译/编程/写作/解题答疑/需求规格/文本润色/内容摘要/文案生成/工作总结/提示词生成/人物档案/角色扮演/深度研究）、15 个内置工具（管理笔记/待办/标签/笔记本/JSON 处理/网页读取/分段读取笔记/制定与更新计划等），MCP 服务器扩展（设置页完整管理 + 连接池预热 + 断线重连），每个工具可单独启用/禁用，向量+关键词混合召回
