...
20. **子 Agent 机制（os_agent）**：文件/命令 13 工具（read_file/write_file/edit_file/ls_dir/glob/grep_file/copy_item/move_item/delete_item/mkdir_dir/run_command/run_python/transfer_item）封装为 os_agent 委托工具，**通用机制集中在 [subagent.go](internal/agent/subagent.go)**：delegatedAgentTool 基类 + newDelegatedAgentTool 工厂 + toolConstructors 构造器注册表 + 事件转发循环（InvokableRun），os_agent = 通用工厂 + 一份 subAgentConfig 配置（osAgentConfig，含提示词/白名单/前缀/描述）；**新增子 Agent 开发规范：每 Agent 一个文件（如 [subagent_os.go](internal/agent/subagent_os.go) 样板、subagent_note.go）只定义提示词常量 + 白名单 + subAgentConfig 配置 + 一个构造器（调 newDelegatedAgentTool），无需改通用机制，再在 registry.go buildTools 注册对应委托工具**；父层 buildTools 仅注册 os_agent（`tools.WrapWithError` 包装、受 ai_agent_tools_disabled 过滤、非 PlanOnly、旧禁用名静默忽略）；内层独立 `adk.NewChatModelAgent` 复用同一会话 ChatModel 客户端、`MaxIterations` 由设置项 `ai_sub_agent_max_iterations` 提供（默认 50、范围 1–200，装配时读取）、白名单恰 13 工具；内层每步工具调用经 `emitToolStart/emitToolResult` 写入父层同一 toolRecords 并发射 `ai:tool-status`（顺序 = os_agent start → 内层记录 → os_agent result 构成前端分组边界），内层流式正文/思考链不转发（noopEmit）；内层复用同一会话 Approver/AskWaiter，审批弹窗照常；前端实时与回放共用 `osAgentGroup*` 栈函数 + `nextToolSeq`（组内独立计数：os_agent 主行走全局 seqMap，子步骤走栈顶组内 seqMap，从 1 起不与父层叠加）+ isAgentGroup/substep 标记；明细**按原始顺序逐行渲染不聚合**（成功/失败/部分失败均独立成行，失败行自带原因）；os_agent 组支持折叠/展开——实时默认展开、回放与流结束后默认折叠，点击主行**局部切换**该组子步骤显隐（不整表重建，避免卡顿与选中异常）；折叠态存于模块级 `collapsedOsAgentMap`（WeakMap：listEl → 已折叠下标 Set），随列表 DOM 回收，避免无界增长。
...
| **后端修复（重要）** | [ls_dir.go](internal/agent/tools/ls_dir.go) 与 [delete_item.go](internal/agent/tools/delete_item.go) 的 `fs.ReadDir(root.FS(), rel)` 改为 `fs.ReadDir(root.FS(), filepath.ToSlash(rel))`：`os.Root.FS()` 的 io/fs 适配器在 Windows 上仅接受 `/` 分隔的多层路径，`filepath.Rel` 返回的反斜杠 rel（如 `a\b`）会报 `readdir invalid argument`（单层路径无分隔符恰好逃过，故仅钻取第二层起暴露；`read_file`/`mkdir_dir` 走 os.Root 原生 API 不受影响）；[config.go](internal/config/config.go) `SandboxFilePath` 前置拦截「以 `/` 或 `\` 开头但 `filepath.IsAbs` 为 false」的外来/畸形绝对路径（Windows 上 Unix 风格 `/home/user/...` 曾被当相对路径拼进沙箱根产生幽灵路径、报无诊断性的"文件不存在"致模型反复重试），改为报「路径格式无效」并引导用相对路径；[subagent_os.go](internal/agent/subagent_os.go) 提示词补「不要猜测 ~ 的展开路径，一律用相对工作区的路径」。 |
...
| 记忆点 | 内容 |
| --- | --- |
| **变更概览** | 新增**子 Agent 运行上限**设置项：os_agent 内层 ReAct 循环上限从硬编码 20 改为可配置 `ai_sub_agent_max_iterations`（默认 50，范围 1–200）；同时把主 Agent `ai_agent_max_iterations` 默认值 20→100，并把三处口径（HTML max / 前端 change 校验 / 后端 clamp）统一到 1–500。spec 见 `.trae/specs/add-sub-agent-max-iterations-setting/`。 |
| **后端（重要）** | [db.go](internal/database/db.go) `InitDefaultSettings`：`ai_agent_max_iterations` 种子 `"20"`→`"100"`、新增 `ai_sub_agent_max_iterations`=`"50"`（种子仅增量插入，存量库旧值不被覆盖）；[types.go](internal/services/types.go) `SettingsConfig` 新增 `AISubAgentMaxIterations`（json tag 同名），`GetAllSettings` 默认值 100/50（必须与种子一致），`SaveAllSettings` clamp 主 Agent `<1→100`/`>500→500`、子 Agent `<1→50`/`>200→200` 并在 sets map 写入（漏写会静默丢失）；[agent.go](internal/agent/agent.go) `DefaultMaxIterations` 20→100；[subagent_os.go](internal/agent/subagent_os.go) 常量改名 `osSubAgentMaxIterations`→`osSubAgentDefaultMaxIterations`=50，新增 `subAgentMaxIterations(setting)` 读取函数（nil/非数字/`<1` 回退 50，`>200` 取 200），`buildOSSubAgent` 增第 4 参 `setting *services.SettingService` 并**拷贝 `osAgentConfig` 后覆盖** `maxIterations`（禁止直接改包级 var，防并发 Run 数据竞争）；[registry.go](internal/agent/registry.go) 装配点传 `p.deps.Setting`；[subagent_test.go](internal/agent/subagent_test.go) 8 处调用补 `nil`。**设置即时生效**：`buildTools` 在每次 `Run` 内装配，改完无需重启。 |
| **前端** | [index.html](frontend/index.html)「Agent 运行上限」`value` 20→100，其下新增「子 Agent 运行上限」（`id="aiSubAgentMaxIterations"`、`min=1 max=200 value=50`，复用既有 class，无新增 CSS）；[main.js](frontend/src/main.js) 三处（change 监听 / `loadSettings` 回显 / `saveSettings` 收集）——主 Agent 回退 100、上限校验由 100 放宽到 500，子 Agent 回退 50、校验 `<1→50`/`>200→200`。数字型设置项沿用相邻项的直接 `getElementById` 写法（不注册进 `els`）。 |
