...
| **后端（重要）** | [db.go](internal/database/db.go) `InitDefaultSettings`：`ai_agent_max_iterations` 种子 `"20"`→`"100"`、新增 `ai_sub_agent_max_iterations`=`"50"`（种子仅增量插入，存量库旧值不被覆盖）；[types.go](internal/services/types.go) `SettingsConfig` 新增 `AISubAgentMaxIterations`（json tag 同名），`GetAllSettings` 默认值 100/50（必须与种子一致），`SaveAllSettings` clamp 主 Agent `<1→100`/`>500→500`、子 Agent `<1→50`/`>200→200` 并在 sets map 写入（漏写会静默丢失）；**迭代上限的默认值与取值范围已收敛到 [config.go](internal/config/config.go) 常量**（`AIAgentMaxIterationsDefault/Max`、`AISubAgentMaxIterationsDefault/Max`），供种子初始化 / `GetAllSettings` 默认值 / `SaveAllSettings` clamp / 运行时装配共用，消除同一数值多处硬编码后相互漂移；[agent.go](internal/agent/agent.go) 删除导出常量 `DefaultMaxIterations`，新增共用读取函数 `parseIterationLimit(raw, def, max)`（非数字/`<1` 回退 def，超 max 取 max）与 `iterationLimitFromSetting(setting, key, def, max)`（`setting` 为 nil 直接回退 def），`Run` 改调后者读 `ai_agent_max_iterations`；[subagent_os.go](internal/agent/subagent_os.go) 删除本地常量 `osSubAgentDefaultMaxIterations` 与 `subAgentMaxIterations`，`osAgentConfig.maxIterations` 引用 `config.AISubAgentMaxIterationsDefault`，`buildOSSubAgent` 复用 `iterationLimitFromSetting` 读 `ai_sub_agent_max_iterations`；`buildOSSubAgent` 第 4 参 `setting *services.SettingService` 并**拷贝 `osAgentConfig` 后覆盖** `maxIterations`（禁止直接改包级 var，防并发 Run 数据竞争）；[registry.go](internal/agent/registry.go) 装配点传 `p.deps.Setting`；[subagent_test.go](internal/agent/subagent_test.go) 8 处调用补 `nil`，另补 `TestParseIterationLimit`（8 组表驱动）/`TestIterationLimitFromSettingNil`/`TestSubAgentMaxIterationsFromSetting`（内存 SQLite 验证装配读取：缺键回退、取配置值、超上限钳制）。**设置即时生效**：`buildTools` 在每次 `Run` 内装配，改完无需重启。 |
