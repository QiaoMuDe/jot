...
> 文件/命令工具（read_file 等 13 个）已封装为 `os_agent` 子 Agent（委托工具，见 [subagent_os.go](internal/agent/subagent_os.go)）：内层工具审批语义沿用父层——同一会话 `Approver`（`agentSession.RequestApproval`），危险操作照常发射 `ai:tool-approval` 阻塞确认，前端审批面板零改动；内层每步工具调用以 `ai:tool-status` 实时转发（事件说明见 [EVENTS.md](internal/agent/EVENTS.md) §3.1）。
