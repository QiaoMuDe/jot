...
配置文件位于用户家目录下 **`~/.jot/mcp/`**，默认文件名：

```
mcp-servers.json
```

**首次启动自动初始化**：应用启动时若 `~/.jot/mcp/` 目录或 `mcp-servers.json` 不存在，会自动创建目录并生成默认配置（空 `servers` 列表）；已存在的文件不会被覆盖。

应用每次发起 Agent 对话时都会重新读取该文件并连接启用的服务器，**修改配置后无需重启应用**，下一次对话即生效。每台服务器的连接与握手有 **10 秒超时**（`mcpserver.ConnectTimeout`），超时或连接失败仅跳过该服务器，不阻塞其他服务器与整轮对话。
...
- **单条配置非法**：仅跳过该条服务器，**其余合法服务器正常装配**。每次对话装配时逐条输出 `Warn` 告警，错误信息带索引定位，例如：
  ```
  MCP 服务器配置校验失败，该服务器已跳过  path=~/.jot/mcp/mcp-servers.json  error="server[1]: MCP 服务器 配置非法: name 不能为空"
  ```
...
| 现象 | 排查方向 |
|------|----------|
| 日志无任何 MCP 记录 | 配置文件缺失 → 检查 `~/.jot/mcp/` 是否存在 `mcp-servers.json`（`Debug` 日志） |
