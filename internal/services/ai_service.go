...
// 返回是否实际更新了摘要。
func (a *AIService) UpdateSessionSummary(ctx context.Context, sessionID uint, windowSize int) bool {
	...
	if totalNonSystem <= int64(windowSize) {
		return false
	}

	// 只有新增消息数达到阈值才更新（SummaryMsgCount 记录上次更新时的总消息数）
	...
	// 从上次摘要覆盖范围之后开始取消息
	// 首次摘要时 SummaryMsgCount=0，offset 为 0
	// 后续增量时 offset = 上次总消息数 - keepTail
	offset := session.SummaryMsgCount - keepTail
	if offset < 0 {
		offset = 0
	}
	limit := summarizeUpTo - offset
	...
	newSummary := a.GenerateSessionSummary(genCtx, session.SummaryContent, newMsgs)
	if newSummary == "" {
		return false
	}

	// SummaryMsgCount 记录当前总消息数，而非 summarizeUpTo
	// 这样下次判断时 diff = totalNonSystem - SummaryMsgCount 即为新增消息数
	if err := a.db.Model(&session).Updates(map[string]interface{}{
		"summary_content":   newSummary,
		"summary_msg_count": int(totalNonSystem),
	}).Error; err != nil {
	...
	a.logger.Infow("会话摘要已更新",
		...
		fastlog.Int("summary_msg_count", int(totalNonSystem)))
	return true
}
