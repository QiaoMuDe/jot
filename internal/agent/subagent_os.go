...
// delete_file/mkdir_dir/run_command/run_python/transfer_file 这 13 个文件/命令工具
...
// 工具家族 + run_python + transfer_file）。顺序即注册顺序。
...
var osAgentConfig = subAgentConfig{
	...
	infoDesc:      "将文件与命令类任务委托给操作系统子 Agent 执行。当任务需要在工作区（~/.jot/workspace）内读写文件、查找内容、编辑、复制/移动/删除、创建目录、执行命令、运行 Python 代码/脚本, 或与用户桌面交换文件（下载/上传）时调用；内层子 Agent 会自主规划并调用 read_file/write_file/edit_file/ls_dir/glob/grep_file/copy_file/move_file/delete_file/mkdir_dir/run_command/run_python/transfer_file 完成。request 为完整任务描述（一句话说明目标、路径与约束）。仅处理工作区内的文件与命令诉求及与桌面的文件交换；笔记、网络等其它诉求请直接使用对应工具, 不要委托给本工具。",
