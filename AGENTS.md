...
8. **前端 Agent 工具状态展示**：`ai:tool-status` 事件实时展示工具调用状态条（含 MCP 工具，`ActionText` 文案"调用 {服务器} 的 {工具}"）；`ai:ask-user` 反问面板；`ai:agent-result` 回传搜索来源/召回卡片/工具调用链供 `stream-done` 落库与渲染。**工具调用记录重设计（修复「≥2 报错看不到」）**：旧实现按「工具名」折叠一行，同名再调用成功后用 ✓ 覆盖先前失败行、而失败计数只增不减 → 「徽标 K 失败但无 ❌ 可见」。重构为**逐调用记录** `toolRecords`（{name, seqName, status: running|ok|error|partial, action_text, result, id, startAt, endAt}）：`ai:tool-status` 每次 `tool_start` append 新记录、终态收口同名最近 `running`（无匹配时**防御补行**，与回放语义一致）；明细**成功按名聚合「名 ×N」、失败/部分失败逐条置前各带原因**（不分组分节，顺序即优先级）；header 徽标计数由记录重算、恒等于可见行数。实时与历史回放**共用模块级** `buildToolRecords`/`rebuildToolSummaryHeader`/`buildToolStatusRows`/`updateToolSummary`，`renderToolCalls`（历史）用 `buildToolRecords` 重建 records 走同一渲染——所见即所存，落库 `tool_calls` 格式不变、老消息判空逻辑不变。running 行实时计时收敛为**单一定时器** `toolStatusTimer`（就绪即启、完成即清，防 per-row 泄漏）。详见 [ai-chat.js](frontend/src/js/ai-chat.js)（ensureToolSummary/refreshToolStatus/updateToolSummary）、[ai-chat.css](frontend/src/css/components/ai-chat.css)（.ai-tool-status-item.is-error 强调：--error-bg 底 + --danger 工具名）。
...
19. **Agent 内置工具治理（黑名单开关 + 防御加固 + 写操作强制确认 + ask_user 强制调用）**：`ai_agent_tools_disabled` 黑名单（默认全注册，注册级过滤）控制内置工具；设置页「Agent 工具」下拉多选；Rnx.toml `fclint`（lint + validate:html）挂 frontend run_after。**防御加固**：`WrapWithError.InvokableRun` defer recover（panic→fail 不中断 ReAct）+ web_search goroutine recover；各工具补 `ctx.Err()` 检查；统一文本长度上限（`validateTextLen`：500/find 2000/正文 20000 rune）；browse_notes view 分段读取 offset 整数校验（原 manage_note 的 view，拆分后属只读工具 browse_notes）；manage_tag TrimSpace；read_url `isPrivateHost` SSRF 防护（拒绝 loopback/私网/链路本地含 169.254.169.254/.local/.internal）；manage_note 扩展名校验。**写操作强制确认**：manage_note update/edit/pin/move/add_tag/remove_tag 六动作须带 `confirm=true`，否则返回引导文本提示 ask_user。**ask_user 强制调用**：信息模糊/多方案/需确认三类场景必须调用（app.go 提示词 + 工具 Desc 双通道约束）。详见 [registry.go](internal/agent/registry.go)、[context.go](internal/agent/tools/context.go)、[manage_note.go](internal/agent/tools/manage_note.go)、[browse_notes.go](internal/agent/tools/browse_notes.go)、[ask_user.go](internal/agent/tools/ask_user.go)、[read_url.go](internal/agent/tools/read_url.go)、[manage_tag.go](internal/agent/tools/manage_tag.go)、[app.go](app.go)、[Rnx.toml](Rnx.toml)

20. **manage_note 写模式扩展 + 新工具 read_url / read_note_section（已并入 view）+ meta.go 描述修正**：manage_note 新增 update（标题/扩展名，非空才更新对应字段）、edit 双模式互斥（find+replace 片段替换可作删除（find 优先精确匹配、空白差异自动归一化兜底，count 指定第几次、replace_all=true 全替换且与 count>1 互斥）/ line_start+line_end 行级替换含末尾追加（行号来自 browse_notes 的 view 的 line_numbers=true 输出，replace 空串即删行区间，line_start 大于总行数时为末尾追加语义））、file_ext 缺省 .md；read_url 基于 eino-ext URL Loader 抓取网页提取正文按 `ai_web_search_max_chars` 截断（仅放行 http/https、15s 超时、浏览器 UA）；read_note_section 分段续读大笔记（id/offset/length，view 超大截断时给出续读指引，line_numbers=true 输出全局行号）——**后已并入 manage_note 的 view（offset/length 参数分段续读）→ 再连同 list 独立为只读工具 browse_notes（见记忆点46，manage_note 收窄为纯写，读操作统一走 browse_notes）**，独立工具 read_note_section 已删除；browse_notes 的 view line_numbers 行号前缀仅作行级编辑寻址坐标、不属于正文（复制片段用于 find 时不得包含行号）；meta.go Label 修正——工具描述与实际实现必须一致（曾声称有"删除"能力实际没有，已删除虚假描述，教训）。详见 [manage_note.go](internal/agent/tools/manage_note.go)、[browse_notes.go](internal/agent/tools/browse_notes.go)、[read_url.go](internal/agent/tools/read_url.go)、[meta.go](internal/agent/tools/meta.go)
...
45. **主题系统迭代（新增 mono 单色 + 调深 dark/dracula 暗色）+ 主题名有效性兜底机制**：新增 `mono`「单色」纯灰度零彩色主题（曾新增 carbon 极夜/sakura 樱粉，预览后因配色不够理想已移除，仅保留 mono）；dark/dracula 两个暗色主题 bg/card/topbar/border/divider/hover/input 整族下沉亮度（dark `--bg` #18181C→#0F0F12、dracula #232530→#1A1B25，解决"背景不够深、泛白"）。**主题名有效性兜底**——[theme-config.js](frontend/src/js/theme-config.js) 新增 `resolveTheme(themeName)` 纯函数（`Object.prototype.hasOwnProperty.call(themeLabels,...)` 防 `__proto__`/`constructor` 原型污染，无效→default）；[main.js](frontend/src/main.js) `loadSettings` 在拿到 cfg 后校验，`corrected` 时用**修正后的完整 cfg 整包 `SaveAllSettings` 落库**（保留字体/排序等其他设置原值）+ `nm.show(...,'warning')` 通知；[index.html](frontend/index.html) inline FOUC 脚本用 `Object.keys(criticalColors)` 作有效集、无效立即写回 localStorage（getItem/setItem 均 try/catch 防隐私模式）。**教训**：`applyTheme` 保持纯 DOM 同步器、不落库——早期版曾在其中调 `saveSettings()` 读 UI 默认值覆盖其他设置（严重 bug，值必须以完整 cfg 整包落库）。**FOUC 三源色值同步修复**：variables.css / criticalColors / main.go `themeBG` 曾 7 主题漂移（内联 `:root` 特异性 (0,0,1) 低于 `[data-theme]` (0,1,0)，值不同致启动闪色），统一以 variables.css 为权威源，同步 12/12 主题三源对齐；`SaveAllSettings` 失败改 fire-and-forget 不阻塞后续加载；空值通知文案 fallback `(空值)` 避免 `主题""已失效`。主题维护以 [frontend/src/css/theme-maintenance.md](frontend/src/css/theme-maintenance.md) 为权威文档。详见 [variables.css](frontend/src/css/variables.css)、[theme-config.js](frontend/src/js/theme-config.js)、[main.js](frontend/src/main.js)、[index.html](frontend/index.html)、[main.go](main.go)
...
## 记忆点 1：Agent 工具列表分组显示（AI 助手浮层 + 设置页统一四分组 + 组标签盲切 + 回弹圆角）
...
## 记忆点 2：AI 代理执行基础设施（workspace 工作目录 + 会话审批模式 ApprovalMode + 前端审批选择器）
...
## 记忆点 3：AI 工作目录文件/命令工具 + 审批暂停/续跑（Approver + ai:tool-approval + ApproveToolCall + 三模式）

| 记忆点 | 内容 |
|--------|------|
| **变更概览** | 在记忆点 2 的基础上，正式落地 AI 在 `~/.jot/workspace/` 沙箱内的**文件操作 + 命令执行工具**与**审批暂停/续跑机制**，接入会话审批模式。共五个工具，遵循**一文件一工具**（`tools/` 下 `fs_base.go` 共享基础 + `read_file.go`/`write_file.go`/`ls_dir.go`/`glob.go`/`run_command.go`）：`read_file`（rune 分页，offset/length 续读）、`write_file`（覆盖触发审批、自动建父目录）、`ls_dir`（**单层像 ls**，钻取以目录作 path 再调，用 `os.ReadDir` 非递归）、`glob`（通配符 `*`/`?`/`[abc]` 单层匹配，标准库 `path.Match`，不递归）、`run_command`。`run_command` 采用 **os/exec 裸命令 + 参数数组**（刻意**不支持 shell 语法**——规避 bash/pwsh 语法错位、降低越权逃逸面），`exec.CommandContext` 30s 超时，`exec.LookPath` 找不到命令回填「环境中没有命令」让模型按「报错自适应」改用已有命令或文件工具（**不禁止、不预判**）。 |
...
| **审批分级（重要）** | 危险操作按风险分级：`copy_file` 覆盖（`overwrite=true`）→ `critical=false` 常规审批、新建不审批；`move_file` **始终**审批（移动必移除源，结构性变更，`critical=false`）；`delete_file` 强制 `critical=true`（confirm_every/review 模式强制确认，auto 模式自动放行并留审计痕）；`mkdir_dir`/`write_file` 新建不审批。`requestApproval` 保持 `ctx==nil`（测试裸工具）放行、`Approver==nil` fail-fast。 |
...
## 记忆点 5：审批三模式行为细化 + auto 完全访问自动放行审计留痕（tool_auto_approval 双写+渲染）+ 高危命令验证升级

| 记忆点 | 内容 |
|--------|------|
| **变更概览** | 在记忆点 3/4 审批门控基础上细化三模式行为，核心变更：**auto（完全访问）模式不再拦截高危（黑名单命中）操作**——`critical=true` 也自动放行，但写独立审计留痕 `tool_auto_approval`（区别于普通 tool_approval），并发射 `ai:tool-status` 事件驱动前端渲染独立警示行。原「critical 任何模式都不可绕过」的语义已废除，auto 模式下高危命令改由「留痕 + 前端警示」兜底而非阻塞。另含：写入内容上限上调、高危命令判定增强、审批选择器前端细节。 |
| **三模式门控（重要）** | [agent.go](internal/agent/agent.go) `RequestApproval` 门控公式：`needConfirm := mode=="confirm_every" || (critical && mode=="review")`。**confirm_every**=每次命令执行都阻塞确认；**review**=普通命令自动放行、黑名单命中命令强制确认（不可绕过）；**auto**=普通命令与黑名单命令**全部自动放行**，其中黑名单命中者额外 `recordAutoApproval`（[agent.go](internal/agent/agent.go) `recordAutoApproval`）双写：`appendRecord` 落库 `tool_auto_approval` 审计记录（`Result` 标注「完全访问模式自动放行（高危命令，未经人工确认）」）+ `emit("ai:tool-status")` 实时推送（action=`tool_auto_approval`、`action_text`=完整命令、`result`=留痕说明）。非法 mode 回落 confirm_every。此处 auto 与 review 行为不再等价：仅 auto 对黑名单命令放行并留痕。 |
| **前端渲染（重要）** | 实时路径 `ai:tool-status` 回调与历史回放 `buildToolRecords` 均识别 `tool_auto_approval`：**升级同名最近 running/pending 记录为 `status:'auto'` 警示行**（含完整命令），无配对时新建 auto 行——避免同一命令「警示行 + 普通进度行」双行重复。`tool_result`/`tool_error` 补行前检测同名最近记录已为 auto 则跳过（same 双行防护）。`buildToolStatusRows` 对 `status==='auto'` 渲染独立高亮行（`.is-auto` 警示样式，不再复用 partial 的「部分失败」语义），行内只显示命令；悬停卡 `fillToolTip` 对 auto 标题为「完全访问自动放行」、正文展示完整命令。CSS 新增 `.ai-mode-tip.anchor-right`/`.ai-mode-tip.anchor-left`（[ai-chat.css](frontend/src/css/components/ai-chat.css)）——审批下拉选项目悬停信息卡右/左弹出、箭头朝向随倒置（右弹箭头朝左、左弹箭头朝右、垂直居中），`--tip-arrow-y` 定位。审批模式按钮图标随模式切换：`APPROVAL_MODE_ICON`（confirm_every=锁、review=盾牌对勾、auto=盾牌警告）经 `syncApprovalToggle` 用可信常量 innerHTML 替换（key 经 approvalMode 索引，非法 fallback confirm_every，无注入风险）；描述文案随模式改准确（confirm_every=每次操作需确认、review=常规自动+高风险询问、auto=无审批直接运行+高风险仅提示）。 |
| **长度上限与高危判定（重要）** | [context.go](internal/agent/tools/context.go) `maxToolLongText` 由 20000 上调为 **100000**（rune 计数），统一影响 `write_file.content`/`edit_file.replace`/`http_request.body` 三处（`maxAIMessageChars`=20000 为互相独立的另一护栏，单条 AI 用户消息）。高危命令判定增强：`matchArgTokens` 增加 `--flag=value` 带值参数拆 `"="` 前 flag 命中检查（堵 `--force=x`/`--yes=1` 绕过整词黑名单）；`commandBaseName` 去后缀补充 `.ps1`（与 .exe/.bat/.cmd 一致），使 `script.ps1` 基名归一命中高危集合。 |
| **审查修复与测试（重要）** | 全面代码审查（子代理只读交叉验证，`go build`/`go vet`/`go test`/`npm run build` 全绿）后修复：① 各文件 critical「不可绕过」过时注释与 auto 放行语义对齐（[agent.go](internal/agent/agent.go) `RequestApproval` 文档、[context.go](internal/agent/tools/context.go) `Approver` 接口、[run_command.go](internal/agent/tools/run_command.go) 文件头+审批检查点、[TOOLS.md](internal/agent/TOOLS.md)）；② 回补 `auto_approval` 前端过时注释（[ai-chat.js](frontend/src/js/ai-chat.js) 注明 maxToolLongText=100000 独立于 MAX_AI_INPUT_CHARS）；③ 补 `TestRequestApprovalModeGating`（[approval_test.go](internal/agent/approval_test.go)）对 `ai:tool-status`(tool_auto_approval) 审计事件发射的断言，并修正 auto 分支注释/未声明变量问题。 |
| **涉及文件** | [internal/agent/agent.go](internal/agent/agent.go)（`RequestApproval` 门控 + `recordAutoApproval` 双写）、[internal/agent/tools/context.go](internal/agent/tools/context.go)（`maxToolLongText`=100000）、[internal/agent/tools/run_command.go](internal/agent/tools/run_command.go)（`matchArgTokens` flag=value + `commandBaseName` .ps1）、[internal/agent/approval_test.go](internal/agent/approval_test.go)（事件断言）、[frontend/src/js/ai-chat.js](frontend/src/js/ai-chat.js)（`tool_auto_approval` 实时/回放两路径渲染 + seqName 统一 + 命令高亮 + 审批图标 + tip 左/右弹定位）、[frontend/src/css/components/ai-chat.css](frontend/src/css/components/ai-chat.css)（`.is-auto` + `.anchor-right/.anchor-left`）、[frontend/index.html](frontend/index.html)（审批选项描述/图标/tip 内容）、[internal/agent/TOOLS.md](internal/agent/TOOLS.md)（审批接入规范同步） |
