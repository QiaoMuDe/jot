...
- 原生撤销/恢复（Ctrl+Z/Y）失效 → 需要 CM6 history 模块托管
...
> **注意**：CM6 的 `EditorView` 需要容器可见且具尺寸才能正确测量布局。`openEditor()` 中编辑器面板通过动画渐入（`modalEnter` ~300ms），因此 `initCodeMirror()` 调用时机需延迟到面板动画完成后，或初始化后调用 `cmEditor.requestMeasure()` 重新计算布局。
```
...
**10. CSS 变更 (`frontend/src/style.css`)**
...
a. .cm-editor 容器样式（替代 .editor-textarea）
   ...
   - font-family 继承 var(--font-family)，font-size 继承 var(--font-size-base)
   - 圆角、边框等视觉属性继承
```

> **注意**：当前 CSS 用 `.editor-overlay[data-mode="preview"] .editor-textarea { display:none }` 控制编辑/预览切换。替换 CM6 后，这些选择器中的 `.editor-textarea` 需改为 `.cm-editor`，否则模式切换将失效。
...
b. CM6 Theme 定义（~50 行）
   ...
   - --cm-font-family: var(--font-family)    /* 与应用字体设置联动 */
   - --cm-font-size: var(--font-size-base)    /* 与应用字号设置联动 */
...
| `frontend/src/main.js` | **大改** | 新增 ~120 行 CM6 模块, 删除 ~636 行 (FindReplaceManager), 修改 ~15 个函数 |
...
| 当前功能 | 变更后 | 说明 |
|----------|--------|------|
| 撤销/恢复 | ✅ CM6 内置 | CM6 history extension 接管 |
...
- 移除对 `FindReplaceManager.reset()` 的调用
