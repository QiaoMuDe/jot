...
func seedNotebooks(db *gorm.DB) []models.Notebook {
	...
	if result.Error != nil {
		// 手动创建默认笔记本（跳过 AutoMigrate 的自动创建）
	...
	for _, name := range notebookData {
		...
		nb := models.Notebook{Name: name, SortOrder: 0}
...
func seedNotes(db *gorm.DB, tags []models.Tag, notebooks []models.Notebook) []models.Note {
	...
	for _, nd := range noteData {
		...
		if notebookID == 0 {
			// 如果 NotebookName 为空或未找到，使用默认笔记本的 ID
