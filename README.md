...
- **AI 助手** — eino 驱动，OpenAI 兼容端点，Chat / Agent / Plan 三态模式（Chat 纯对话、Agent 自动调度工具、Plan 先规划后执行），流式输出、深度思考、13 项 AI 技能（翻译/编程/写作/解题答疑/需求规格/文本润色/内容摘要/文案生成/工作总结/提示词生成/人物档案/角色扮演/深度研究）、16 个内置工具（管理笔记/待办/标签/笔记本/JSON 处理/网页读取/HTTP API 调用/分段读取笔记/长期记忆增删改查/制定与更新计划等），跨会话全局记忆空间（辅助记忆自动注入问答，可让 AI 随时记住/更新/遗忘长期偏好），MCP 服务器扩展（设置页完整管理 + 连接池预热 + 断线重连），每个工具可单独启用/禁用（核心记忆工具常驻不可禁用），向量+关键词混合召回
...
**前置依赖**：Go ≥ 1.26、Wails CLI v2.12+、Node.js ≥ 16
