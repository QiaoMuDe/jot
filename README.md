...
- **Chat / Agent 双模式** — Chat 模式专注问答，Agent 模式自动调度内置工具完成多步任务；信息不足时主动向用户反问澄清（单选/多选/自定义）
- **10 个内置工具 + 设置页开关** — Agent 内置管理笔记/待办/标签/笔记本、联网搜索、笔记召回、数据统计、反问确认等 10 个工具，设置页「对话与搜索 → Agent 工具」可单独启用/禁用
- **外部 MCP 工具扩展** — 基于 eino-ext mcp + mark3labs/mcp-go，支持接入任意 MCP 工具服务器（stdio / SSE / HTTP），扩展 Agent 能力边界
- **卡片召回（混合检索）** — 向量检索（sqlite-vec 余弦距离）+ 关键词检索（SQL LIKE 匹配）双路并行召回，按命中优先级排序（双命中 > 仅向量 > 仅关键词），命中块自动补充相邻块上下文，去重合并后注入 AI 对话；支持按笔记本过滤召回范围
...
- **办公文件智能转换** — 集成 markitdown，上传 `.docx`/`.xlsx`/`.xls`/`.pptx`/`.pdf`/`.epub` 等文件自动转为 Markdown 注入上下文（60s 超时保护）
...
| **MCP 工具** | eino-ext/components/tool/mcp + mark3labs/mcp-go | — | Agent 模式接入外部工具服务器（stdio/SSE/HTTP） |
...
# 前端代码检查（ESLint + HTML 校验）
cd frontend && npm run lint && npm run validate:html
```
...
| **Agent 模式怎么用？** | 在 AI 侧栏底部切换到 Agent 模式，AI 即可自动调度内置工具完成多步任务；可在设置页「对话与搜索 → Agent 工具」开关单个工具，并支持接入外部 MCP 工具服务器 |
