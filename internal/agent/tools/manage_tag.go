...
// 本文件实现 manage_tag 标签管理工具：模型在 ReAct 循环中调用它创建标签、列出标签、
// 更新标签或删除标签，底层复用 services.TagService（GetByName / Create / GetAll / Update / Delete），
// 不感知父包 agent 的事件循环细节。
...
//   - delete：删除标签（id 必填、正整数，仅删标签本身，笔记内容不受影响）。
...
func (m *manageTagTool) Info(_ context.Context) (*schema.ToolInfo, error) {
	return &schema.ToolInfo{
		Name: "manage_tag",
		Desc: "管理标签（标签是笔记的附属属性，用于分类筛选笔记）。当用户要求创建标签、查看标签列表、更新标签或删除标签时调用。边界：manage_tag 管理标签本身（创建/重命名/改色/删除），manage_note 的 add_tag/remove_tag 动作管理笔记与标签的关联；给笔记打标签用 manage_note（action=add_tag），管理标签定义用本工具。通过 action 参数区分动作：create=创建标签（需提供 name 标签名称，可提供 color 颜色，格式 #RRGGBB，缺省 #3b82f6）；list=列出全部标签（无额外参数）；update=更新标签（需提供 id 标签编号，可提供 name 新名称或 color 新颜色，至少其一）；delete=删除标签（需提供 id 标签编号，仅删除标签本身，笔记内容不受影响）。返回标签列表或操作结果，列表中的编号 [数字] 可用于后续笔记标签操作或 update/delete。写操作（update）将按当前审批模式弹出确认面板，用户批准后才执行；delete 属高危操作，同样需按审批模式确认。",
		ParamsOneOf: schema.NewParamsOneOfByParams(map[string]*schema.ParameterInfo{
			"action": {
				Type:     schema.String,
				Desc:     "要执行的动作：create=创建标签；list=列出全部标签；update=更新标签；delete=删除标签",
				Enum:     []string{"create", "list", "update", "delete"},
				Required: true,
			},
			"id": {
				Type:     schema.Number,
				Desc:     "标签编号（正整数，列表中的 [数字] 即为 id），action=update / delete 时必填",
				Required: false,
			},
			"name": {
				Type:     schema.String,
...
func (m *manageTagTool) InvokableRun(ctx context.Context, argumentsInJSON string, _ ...tool.Option) (string, error) {
	...
	case "delete":
		...
		if args.ID <= 0 {
			return "", errors.New("manage_tag 删除标签缺少有效的 id")
		}
		// 非整数 id（如 1.5）直接报错而非静默截断，与删除动作的「正整数」参数约定一致
		if float64(uint(args.ID)) != args.ID {
			return "", errors.New("manage_tag 删除标签的 id 须为正整数")
		}
		// 删除为高危写操作：critical=true，按当前审批模式门控（仅删标签本身，笔记内容不受影响）
		if err := m.requestApproval(ctx, fmt.Sprintf("删除标签 #%d", int(args.ID)), true); err != nil {
			return "", err
		}
		if err := m.tag.Delete(uint(args.ID)); err != nil {
			return "", err
		}
		return fmt.Sprintf("已删除标签 #%d（笔记内容不受影响）", int(args.ID)), nil
	}
